...
async function login(nif, password) {
    ...
    response = await client.post('/txempbs/LoginDNISCA.doLogin.bs', urlEncodedData, {
        params,
        headers: {
    ...
    response = await client.post('/txempbs/LoginDNISCA.setLogged.bs', urlEncodedData, {
        params,
        headers: {
...
async function createTransfer({sourceAccount, destinationAccount, amount, nif, beneficiary, concept}, sid) {
    function getFormFields(html) {
        ...
        const ibanJsCodeMatch = html.match(/if \(ibanLabelClean.length < 1\) {([^}]+)}/);
        const reFields = /\("#(_[A-Z0-9]+)"\)/g;
        let ibanFieldMatch, ibanFieldArray = [];
        while ((ibanFieldMatch = reFields.exec(ibanJsCodeMatch[1]))) {
            ibanFieldArray.push(ibanFieldMatch[1]);
        }
        const [ibanField_4_8, ibanField_8_12, ibanField_12_14, ibanField_14] = ibanFieldArray;
        console.log(`IbanFields found: ${ibanFieldArray}`);

        const amountJsCodeMatch = html.match(/function checkInputAmount\(\) {([^}]+)}/);
        ...
        const conceptFieldMatch = html.match(/function setNominaPurpose\((_[A-Z0-9]+)\)/);
        const conceptField = conceptFieldMatch[1];
        console.log(`conceptField found: ${conceptField}`);

        const detailFieldMatch = html.match(/if\(g=="DETALLE"\)document.getElementsByName\("(_[A-Z0-9]+)"\)/)
    ...
    response = await client.post('/txempbs/CUGetSender.init.bs', urlEncodedData, {
        headers: {
            "Accept": "*/*",
            "Content-Type": "application/x-www-form-urlencoded",
        }
    ...
    data = {
        ...
        accountInputType: "3",
        "evision.userLang":               "",
        "evision.RSADevicePrint":         "version%3D3%2E5%2E1%5F4%26pm%5Ffpua%3Dmozilla%2F5%2E0%20%28x11%3B%20linux%20x86%5F64%3B%20rv%3A68%2E0%29%20gecko%2F20100101%20firefox%2F68%2E0%7C5%2E0%20%28X11%29%7CLinux%20x86%5F64%26pm%5Ffpsc%3D24%7C1920%7C1080%7C1080%26pm%5Ffpsw%3D%26pm%5Ffptz%3D1%26pm%5Ffpln%3Dlang%3Des%2DES%7Csyslang%3D%7Cuserlang%3D%26pm%5Ffpjv%3D0%26pm%5Ffpco%3D1%26pm%5Ffpasw%3D%26pm%5Ffpan%3DNetscape%26pm%5Ffpacn%3DMozilla%26pm%5Ffpol%3Dtrue%26pm%5Ffposp%3D%26pm%5Ffpup%3D%26pm%5Ffpsaw%3D1920%26pm%5Ffpspd%3D24%26pm%5Ffpsbd%3D%26pm%5Ffpsdx%3D%26pm%5Ffpsdy%3D%26pm%5Ffpslx%3D%26pm%5Ffpsly%3D%26pm%5Ffpsfse%3D%26pm%5Ffpsui%3D%26pm%5Fos%3DLinux%26pm%5Fbrmjv%3D68%26pm%5Fbr%3DFirefox%26pm%5Finpt%3D%26pm%5Fexpt%3D",
        "evision.csid": sid,
        "evision.RSADeviceFso":           "",
        checkAJ: "S",
        ...
        "beneficiaryAccount.iban.countryCode": destinationAccount.substring(0,2),
        "beneficiaryAccount.iban.checkDigit": destinationAccount.substring(2,4),
        "beneficiaryAccount.iban.accountNumber": destinationAccount.substring(4),
    ...
    data.accHash = Aes.Ctr.encrypt(destinationAccount.substring(4), beneficiary, 256)
    urlEncodedData = qs.stringify(data);
    response = await client.post('/txempbs/TRExternalTransfer.reinit.bs', urlEncodedData, {
        headers: {
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
            "Content-Type": "application/x-www-form-urlencoded",
        }
    ...
    urlEncodedData = qs.stringify(data);
    response = await client.post('/txempbs/TRExternalTransfer.password.bs', urlEncodedData, {
        headers: {
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
            "Content-Type": "application/x-www-form-urlencoded",
        }
